    ...
    with sync_playwright() as p:
        ...
        for section_id in section_ids:
            ...
            if (os.path.exists(screenshot_path)):
                print(f"{screenshot_path} exists so created new {section_id}_{counter}.png")
                screenshot_path = os.path.join(output_dir, f"{section_id}_{counter}.png")
                counter += 1
...
def capture_sections_svgs_only(html_path: str, output_dir: str = "svgs", wait_ms: int = 1000):
    """
    Captures SVGs from all <section id="..."> elements containing Mermaid diagrams.

    Args:
        html_path (str): Absolute file URL to the HTML file, e.g., "file:///C:/path/to/app.html"
        output_dir (str): Folder to save SVGs.
        wait_ms (int): Time to wait after navigation to allow rendering (in milliseconds).
    """
    os.makedirs(output_dir, exist_ok=True)
    ...
    with sync_playwright() as p:
        ...
        for section_id in section_ids:
            ...
            # Extract the <svg> inside this section
            locator = page.locator(f"section#{section_id} svg")
            if locator.count() == 0:
                print(f"No SVG found in section {section_id}, skipping.")
                continue

            svg_content = locator.evaluate("el => el.outerHTML")

            # Save SVG file
            svg_path = os.path.join(output_dir, f"{section_id}.svg")
            counter = 1
            while os.path.exists(svg_path):
                svg_path = os.path.join(output_dir, f"{section_id}_{counter}.svg")
                counter += 1
...
if __name__ == "__main__":
    ...
    html_file5 = "./test/test-answers/answers-workflow1.html"
    ...
    capture_sections_svgs_only(file_url, output_file)
    print(f"Finished caturring screenshots!")